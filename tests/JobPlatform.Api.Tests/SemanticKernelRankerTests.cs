...
/// <summary>
/// The JSON extraction the Semantic Kernel ranker depends on.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class SemanticKernelRankerTests
{
    private const string Body = "{\"matches\":[]}";

    [Theory]
    // Bare, which is what the prompt asks for.
    [InlineData(Body)]
    // Fenced, which models do anyway.
    [InlineData("```json\n" + Body + "\n```")]
    [InlineData("```\n" + Body + "\n```")]
    // Prefixed with prose.
    [InlineData("Here are the rankings: " + Body)]
    ...
    {
        var json = SemanticKernelCvRanker.ExtractJsonObject(response);
    ...
    {
        const string payload = "{\"matches\":[{\"postingId\":1,\"score\":90}]}";

        var json = SemanticKernelCvRanker.ExtractJsonObject("Ranked: " + payload + " done.");
    ...
    [InlineData("")]
    [InlineData("   ")]
    [InlineData("I cannot rank these postings.")]
    [InlineData("}{")]
    public void A_response_with_no_json_object_yields_null_rather_than_throwing(string response)
        => Assert.Null(SemanticKernelCvRanker.ExtractJsonObject(response));
}
