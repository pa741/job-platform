    ...
    IApplicationWriter? writer = null)
{
    ...
    [Description(
        ...
        + "it means two different things depending on 'channel'. On an 'Ats' row the board has "
        + "said the application happens on the employer's own system without publishing the "
        + "address, so the employer's form is one hop away: open the posting page and follow its "
        + "apply link. On any other row it is a link into a second search results page, and "
        + "following one spends a day's cap arriving nowhere. Pass excludeAggregators=true to "
        + "have the server drop every 'Aggregator' row, the followable ones included, rather "
        + "than skipping them here: a filter applied to rows already returned shrinks the batch "
